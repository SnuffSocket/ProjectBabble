...
class Camera:
    def __init__(
            ...
            camera_output_outgoing: "queue.Queue(maxsize=3)",
    ...
    def get_cv2_camera_picture(self, should_push):
        try:
            ...
            self.fps = 0.2 * current_fps + 0.8 * self.fps
    ...
    def reflect_data(self, x):
        x = ((x & 0x55) << 1) | ((x & 0xAA) >> 1)
        x = ((x & 0x33) << 2) | ((x & 0xCC) >> 2)
        return ((x & 0x0F) << 4) | ((x & 0xF0) >> 4)

    def crc8(self, data, crc=0xFF):
        for d in data:
            crc ^= self.reflect_data(d)
            for _ in range(8):
                crc <<= 1
                if crc & (1 << 8):
                    crc ^= (1 << 8 | 0x07)
        return self.reflect_data(crc) ^ 0xFF
                ...
                if buffer_len >= ETVR_HEADER_LEN:
                    ...
                    if buffer_len >= end and self.buffer[end-4:end-2] == b'\xff\xd9':
                        ...
                        if self.buffer[end-2:end] != b'\xff\xa0':
                            if self.buffer[end-2:end-1] != int.to_bytes((self.crc8(self.buffer[ETVR_HEADER_LEN:end-2])), signed=False, byteorder="little"):
